...
import info.mobilesgmc.HomeActivity;
...
public class WebServiceUtils {
    ...
    public static Integer guardarCirurgia(Cirurgia cirurgia,
                                         String token) throws ClientProtocolException, IOException,
            ParseException, JSONException, RestClientException {
        int adicionou = 0;
        Gson g = new Gson();

        HttpPost httpPost = new HttpPost(URL + "guardarCirurgia?token="
                + token);
        StringEntity se = new StringEntity(g.toJson(cirurgia, Cirurgia.class),
                "UTF-8");
        ...
        if (isOk(httpResponse.getStatusLine().getStatusCode())) {
            ...
            adicionou = Integer.valueOf(string);
        } else {
            throw new RestClientException(
                    "HTTP Response with invalid status code"
                            + httpResponse.getStatusLine().getStatusCode()
...
//		}
    ...
    public static ArrayList<EspecialidadeTipoCirurgia> getAllEspecialidadesComTipos(String token)
            throws ClientProtocolException, IOException, RestClientException,
            ParseException, JSONException {
        ArrayList<EspecialidadeTipoCirurgia> especialidade = null;

        HttpGet request = new HttpGet(URL + "getEspecialidadesECirurgias?token=" + token);
        // request.setContentType(new BasicHeader(HTTP.CONTENT_TYPE,
        // "application/json"));
        request.setHeader("Accept", "Application/JSON");
        ...
        if (isOk(basicHttpResponse.getStatusLine().getStatusCode())) {
            especialidade = new ArrayList<EspecialidadeTipoCirurgia>();
            Type collectionType = new TypeToken<ArrayList<EspecialidadeTipoCirurgia>>() {
            }.getType();
            especialidade = g.fromJson(
                    EntityUtils.toString(basicHttpResponse.getEntity()),
                    collectionType);
